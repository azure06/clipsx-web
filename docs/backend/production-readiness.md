...
| Production builds depended on fetching Google Fonts through `next/font/google`. | Bundle Space Grotesk and Noto Sans JP through Fontsource variable-font packages with system fallbacks. | After dependency installation, builds require no font-service network access; browsers load font assets from the site itself. |
