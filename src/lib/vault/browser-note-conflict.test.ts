...
describe('browser note conflict resolution', () => {
  ...
  it('reapplies an in-memory local draft against the refreshed remote head', () => {
    ...
    expect(resolved).toMatchObject(local);
  ...
  it('uses the user-provided manual merge without changing the remote item identity', () => {
    ...
    expect(manualMergeResolution(beginNoteConflict(local, remote), { mediaType: 'text/plain', title: 'Reformatted', content: new Uint8Array([1]), labels: [] })).toMatchObject({ mediaType: 'text/plain', title: 'Reformatted' });
  });
});
