...
const DATABASE_NAME = "clipsx-vault-v2"; const DATABASE_VERSION = 3; const STORE_NAME = "vault-settings"; const DEVICE_STORE_NAME = "browser-device-records";
function database(): Promise<IDBDatabase> { if (typeof indexedDB === "undefined") throw new Error("Vault settings require a browser."); return new Promise((resolve, reject) => { const request = indexedDB.open(DATABASE_NAME, DATABASE_VERSION); request.onupgradeneeded = () => { if (!request.result.objectStoreNames.contains(STORE_NAME)) request.result.createObjectStore(STORE_NAME, { keyPath: "accountId" }); if (!request.result.objectStoreNames.contains(DEVICE_STORE_NAME)) request.result.createObjectStore(DEVICE_STORE_NAME, { keyPath: ["accountId", "deviceId"] }); }; request.onsuccess = () => resolve(request.result); request.onerror = () => reject(request.error ?? new Error("Unable to open vault settings.")); }); }
