...
describe('vault collection sync tombstones', () => {
  it('accepts a tombstone only when its signed deletion operation binds the same note and revision head', async () => {
    ...
    const commandBytes = await createItemDeleteCommand({
      ...
      itemId: 'item-1', expectedRevisionHash: revisionHash, operationId: 'operation-1',
    });
    ...
    expect(await verifyProtocolRecord('clipsx/vault/v1/command/item-delete', command.signedBytes, command.signature, signing.publicKey)).toBe(true);
    const sync = encodeCanonicalCbor(new Map<number, CborValue>([
      [1, 1], [2, 'collection-1'],
      [3, [new Map<number, CborValue>([[1, 'operation-1'], [2, 1], [3, 'item-delete'], [4, command.signedBytes], [5, null], [6, await sha256(commandBytes)], [7, 'device-1'], [8, command.signature]])]],
      [4, []], [5, 1], [6, [new Map<number, CborValue>([[1, 'item-1'], [2, revisionHash], [3, 'device-1'], [4, 'operation-1']])]],
