...
describe('browser device registration', () => {
  it('builds a recovery-signed first-device command bound to its HPKE challenge', async () => {
    ...
    const command = await createInitialDeviceRegistrationCommand({
      ...
      displayName: 'Chrome', platform: 'web', enrollmentOrigin: 'http://localhost:3000', protectionProfile: 'vault-passphrase-wrapped', capabilities: utf8('v1'),
    ...
    expect(decodeCanonicalCbor(parsed.payload).get(5)).toBe('http://localhost:3000');
  });
});
