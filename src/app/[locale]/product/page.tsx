  ...
  return <div className={styles.page} lang={locale}><div className={styles.shell}>
    <header className={styles.header}>
      <div><p className={styles.eyebrow}>{c.eyebrow}</p><h1>{c.title}</h1><p className={styles.intro}>{c.intro}</p>
        <div className={styles.actions}><Link href="/download" className={styles.primary}>{c.get}<ArrowRight size={16} aria-hidden="true" /></Link><Link href="/docs/getting-started" className={styles.link}>{c.docs}<ArrowUpRight size={16} aria-hidden="true" /></Link></div>
      </div>
    ...
    <div className={styles.layout}>
      ...
      <div className={styles.article}>
        ...
        <section id="privacy" aria-labelledby="privacy-title" className={styles.section}><p className={styles.eyebrow}>{c.nav[4]}</p><h2 id="privacy-title">{c.privacyTitle}</h2><p>{c.privacyBody}</p><dl className={styles.boundaries}>{c.privacyRows.map(([title, body], index) => { const Icon = privacyIcons[index]; return <div key={title}><dt><Icon size={21} aria-hidden="true" />{title}</dt><dd>{body}</dd></div>; })}</dl><div className={styles.actions}><Link href="/docs/privacy" className={styles.link}>{c.privacyLink}<ArrowUpRight size={16} aria-hidden="true" /></Link><Link href="/docs/sync" className={styles.link}>{c.syncLink}<ArrowUpRight size={16} aria-hidden="true" /></Link></div></section>
