...
export async function openVaultCollectionSync(input: {
  ...
  for (const [pageIndex, pageBytes] of input.pages.entries()) {
    ...
    for (const entry of revisions) {
      ...
      if (!same(await sha256(signed), revisionHash) || !await verifyProtocolRecord('clipsx/vault/v1/note-revision', signed, signature, signingKey)) throw new Error('Unverified vault revision.');
      const decoded = decodeCanonicalCbor(await decryptRevisionContent({ accountId: input.accountId, collectionId: input.collectionId, noteId: id, epoch: epochNumber, revision: revisionNumber, epochKey: input.epochKey, encryptedContent: { ciphertext, nonce: contentNonce }, wrappedRevisionKey: { ciphertext: wrapped, nonce: wrapNonce } }));
    ...
    for (const entry of tombstones) {
      ...
      const command = known.get(operationId); if (!command || command.get(3) !== 'note-delete' || command.get(5) !== `device:${authorId}`) throw new Error('Unverified vault tombstone.');
