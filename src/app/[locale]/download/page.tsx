        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          {downloadTargets.map((target) => {
            ...
            return (
              <div
                key={target.platform}
                className="flex flex-col items-center text-center rounded-2xl border border-white/8 bg-white/3 p-8 hover:border-cyan-500/30 transition-all"
              >
                ...
                <p className="text-xs text-gray-500 mb-6">{t(target.reqKey as any)}</p>
                <a
                  href={target.url}
          ...
          <ol className="space-y-4">
            {['install_1', 'install_2', 'install_3', 'install_4'].map((k, i) => (
              <li key={k} className="flex items-start gap-4">
                ...
                <span className="text-sm text-gray-400 pt-0.5">{t(k as any)}</span>
              </li>
            ))}
          </ol>
        </div>

        <p className="text-xs text-gray-600 text-center">{t('note')}</p>
      </div>
    </div>
  );
}
