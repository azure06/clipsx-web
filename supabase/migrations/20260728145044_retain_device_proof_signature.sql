-- Preserve both independent proofs required to authorize a browser device:
-- the recovery-root authorization and the proposed device's possession proof.
alter table public.vault_device_authorizations
  add column proof_of_possession_signature bytea;

alter table public.vault_device_authorizations
  add constraint vault_device_authorizations_proof_signature_length
    check (
      proof_of_possession_signature is null
      or octet_length(proof_of_possession_signature) = 64
    );
