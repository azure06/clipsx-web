...
export function Header({ user }: { user: User | null }) {
  ...
  return <header ref={headerRef} className="fixed inset-x-0 top-0 z-50 border-b border-black/5 bg-[rgba(248,250,252,.88)] backdrop-blur-xl dark:border-white/5 dark:bg-[rgba(2,6,23,.88)]">
    <div className="mx-auto flex h-16 max-w-[1440px] items-center px-4 sm:px-6 lg:px-8">
      <Link href="/" onClick={closeNavigation} className="focus-ring flex items-center gap-2.5 rounded-md" aria-label="ClipsX home"><BrandMark className="h-[34px] w-[34px] text-slate-700 dark:text-[#d9e0ea]" /><span className="font-heading text-sm font-bold tracking-[.2em]">CLIPS<span className="text-slate-500 dark:text-slate-400">X</span></span></Link>
