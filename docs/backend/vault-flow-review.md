...
| Unlock and session bind | Worker decrypts device keys locally, verifies the paginated account ledger and signer directory, and then opens bootstrap only at the verified head. | Limits encrypted-record access to an active local device and detects rollback or signer substitution. | Lock/retry locally; an account session alone cannot bypass binding. |
