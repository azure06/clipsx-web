import { createEncryptedRevision, type VaultItemContent } from './encrypted-revision';
import { encodeCanonicalCbor, signProtocolRecord, type CborValue } from './protocol';

export async function createItemAppendCommand(input: {
  ...
  expectedCollectionHead: Uint8Array; content: VaultItemContent;
  itemId?: string; operationId?: string; revisionNumber?: number; previousRevisionHash?: Uint8Array; epochNumber?: number;
}): Promise<{ itemId: string; command: Uint8Array }> {
  ...
  const revision = await createEncryptedRevision({
    accountId: input.accountId, collectionId: input.collectionId, itemId, epoch: epochNumber, revision: revisionNumber,
  ...
  unsigned.set(10, await signProtocolRecord('clipsx/vault/v1/command/item-append', signed, input.deviceSigningSecretKey));
  unsigned.set(11, new Map<number, CborValue>([
    [1, revision.encryptedContent.ciphertext],
    [2, revision.wrappedRevisionKey.ciphertext],
  ]));
  return { itemId, command: encodeCanonicalCbor(unsigned) };
}
/** @deprecated use createItemAppendCommand */
export const createNoteAppendCommand = createItemAppendCommand;
