  ...
  return <header ref={headerRef} className="fixed inset-x-0 top-0 z-50 border-b border-black/5 bg-[rgba(248,250,252,.88)] backdrop-blur-xl dark:border-white/5 dark:bg-[rgba(2,6,23,.88)]">
    <div className="mx-auto flex h-16 max-w-[1440px] items-center px-4 sm:px-6 lg:px-8">
      ...
      <nav aria-label="Primary" className="ml-9 hidden items-center gap-0.5 xl:flex">
        ...
        {mainNavLinks.map((item) => <Link key={item.href} href={item.href} onClick={closeNavigation} aria-current={isActive(pathname, item.href) ? 'page' : undefined} className={`nav-link focus-ring inline-flex items-center gap-1 ${isActive(pathname, item.href) ? 'bg-violet-500/[.08] text-violet-700 dark:text-violet-300' : ''}`}>{t(item.labelKey as never)}{item.href === '/docs' && <ArrowUpRight size={13} />}</Link>)}
      </nav>
      <div className="ml-auto hidden items-center gap-2 xl:flex"><GitHubRepositoryLink className="focus-ring inline-flex h-10 items-center gap-1.5 rounded-[.65rem] px-2.5 text-sm text-slate-600 transition hover:bg-[rgb(148_163_184_/.12)] hover:text-slate-950 dark:text-slate-400 dark:hover:bg-white/6 dark:hover:text-white" label={t('Nav.github')} />{accountControl}<Link href="/download" onClick={closeNavigation} className="focus-ring ml-1 inline-flex h-10 items-center gap-2 rounded-[.65rem] bg-slate-950 px-4 font-heading text-sm font-bold text-white transition hover:bg-violet-700 dark:bg-white dark:text-slate-950 dark:hover:bg-violet-300">{t('Nav.getClipsX')}<ArrowRight size={15} /></Link></div>
